...
function pocketCollar(p, from, to, radius) {
  ...
  const vertices = [], indices = [], steps = 48;
  const crossSection = [
    [radius - 0.019, -0.55], [radius - 0.019, 0.174],
    [radius - 0.016, 0.184], [radius - 0.010, 0.190],
    [radius - 0.003, 0.192], [radius + 0.004, 0.188],
    [radius + 0.010, 0.179], [radius + 0.011, 0.163],
  ];
...
export function buildTournamentTable(scene, renderer) {
  ...
  const leatherRim = new THREE.MeshStandardMaterial({ color: '#171819', roughness: 0.54, side: THREE.DoubleSide });
  ...
  for (const p of POCKET_DETAILS) bed.holes.push(path(p.outline, true));
  ...
  for (const p of POCKET_DETAILS) {
    ...
    const lining = p.outline.map(([x, z]) => {
    ...
    const radius = p.width + 0.021;
    const start = p.side ? Math.asin((0.352 - p.roundCenter) / radius) : 0.85;
    const collar = add(pocketCollar(p, start, Math.PI - start, radius), leatherRim);
    collar.name = 'pocket-rounded-collar';
    const base = add(new THREE.ShapeGeometry(path(p.outline)), pocketBase, -0.63);
    base.rotation.x = -Math.PI / 2;
  }
  const sightMaterial = new THREE.MeshStandardMaterial({ color: '#f0eee1', roughness: 0.26, metalness: 0.10 });
  const sight = (x, z) => {
    const m = add(new THREE.SphereGeometry(0.025, 20, 10), sightMaterial, Y + 0.190);
    m.position.x = x; m.position.z = z; m.scale.set(0.7, 0.10, 1.3); m.castShadow = false;
  };
