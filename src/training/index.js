...
export function mountTraining({physics,scene,enter,home,practice,resetControls,prepareShot,isBusy,changed}){
  ...
  function open(){resetControls();lesson=null;phase='idle';panel.hidden=true;resultBox.hidden=true;resultBox.replaceChildren();overlay.clear();library.hidden=false;renderLibrary();}
  function close(){lesson=null;phase='idle';library.hidden=true;panel.hidden=true;resultBox.hidden=true;resultBox.replaceChildren();overlay.clear();}
  function start(id){
    if(isBusy())return;
  ...
  function renderPanel(message=''){
    ...
    panel.querySelector('[data-exit]')?.addEventListener('click',()=>{if(!isBusy())open();});
